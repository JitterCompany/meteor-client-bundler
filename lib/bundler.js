...
function bundle(options) {
  ...
  var release = options.release || userConfig.release || userConfig.runtime.release;
  ...
  // Compose packages file based on provided config
    ...
    // Lookup meteor package versions in the 'importVersionsFrom' folder
    if(config["importVersionsFrom"]) {
        var sourceDir = Path.resolve(cwd, config["importVersionsFrom"]);
        var sourcePacksFile = Path.resolve(sourceDir, ".meteor/packages");
        console.log("Overriding import versions: versions will be based on '" + sourcePacksFile +"'") 
        var sourcePacksLines = Fs.readFileSync(sourcePacksFile).toString().split("\n");
        var sourceVersions = {}
        for(line of sourcePacksLines) {
            var item = line.split('#')[0].trim().split('@')
            if(item.length >= 2) {
                var lib = item[0]
                var version = item[1]
                sourceVersions[lib] = version
            }
        }

        config["import"] = config["import"].map(lib => {

            var libName = lib.split('@')[0].trim();
            var overrideVersion = sourceVersions[libName]
            if(overrideVersion) {
                console.log("Override import '" + libName +"' to version '" + overrideVersion + "'")
                return libName + '@' + overrideVersion;
            }
            return lib;
